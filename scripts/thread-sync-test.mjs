...
try {
  ...
  assert.match(preparedPopupScript, /!thread\.parentThreadId/,
    "the normal RALPH list excludes reviewer child threads");
  assert.match(preparedPopupScript, /loadedReviews\.filter/,
    "the review section renders jobs from the reviewer registry");
  ...
  assert.match(preparedContentScript, /__local-codex-support-automation/,
    "active page automation exposes an in-page status control");
  assert.match(preparedContentScript, /Local Codex automation stopped by user/,
    "the in-page Stop action aborts the active automation path");
  ...
  const completionBus = new SupportCommandBus();
  const abandonedContinuation = completionBus.execute(
    { feature: "ralph", kind: "send_message", targetUrl: urlA, message: "abandoned continuation" },
    60_000,
    { operation: { label: "Resume RALPH thread", parentThreadId: parseConversationUrl(urlA).threadId } },
  );
  const abandonedContinuationRejected = assert.rejects(abandonedContinuation, /marked complete/i);
  const completeThreadHandler = ralphThreadCompleteHandler(projectScopedRegistry, sync.extensionToken, completionBus);
  ...
  await abandonedContinuationRejected;
  assert.deepEqual(completionBus.operations(), [], "mark complete removes queued automation for that thread");
  completionBus.close();
  ...
  assert.deepEqual(await requestPreparation({ conversationUrl: urlC, canPrepare: false }), {
    code: 200, body: { status: "observed" },
  }, "Helium observes active RALPH threads without opening Chrome");
  assert.equal(preparationLaunches, 0);
  assert.deepEqual(await requestPreparation({ conversationUrl: urlC }), {
    code: 200,
  ...
  assert.match(submitSubagentDefinition.description, /waits for the recorded reviewer to become idle/);
  ...
  assert.match(startSubagentCommand.message, /No reviewer thread sync or URL lookup is needed/);
  ...
  // Submit from a reviewer session that has never synced.
  ...
  const [submitResult, duplicateSubmit] = await Promise.all([
    client.callTool({
      ...
      // Initial submission works without session metadata.
    }),
    client.callTool({
      name: "review_done",
  ...
  assert.equal(submitResult.isError, undefined, "an unsynced reviewer can submit its report");
  ...
  const secondSubagentRetryCall = secondSubagentReplayHandler(subagentRetryArguments, { ...subagentRetryExtra, requestId: "different-request-same-task" });
  await new Promise(resolve => setImmediate(resolve));
...
async function testWorkerNeverRedispatchesAfterLostResponse(sync) {
  ...
  assert.equal(injected, 2, "the health check and command content script is established before the side-effecting dispatch");
