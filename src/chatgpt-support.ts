...
const supportCommandSchema = z.union([
  z.object({
    ...
    operationLabel: z.string().max(120).optional(),
...
interface SupportCommandOperationMetadata {
  label: string;
...
export class SupportCommandBus {
  ...
  async execute(
    ...
    options: { allowBrowserLaunch?: boolean; operation?: SupportCommandOperationMetadata } = {},
  ) {
    ...
    const fullCommand = supportCommandSchema.parse({
      ...command,
      ...(refreshRevision ? { refreshRevision } : {}),
      ...(options.operation?.label ? { operationLabel: options.operation.label } : {}),
      id: randomUUID(),
    });
  ...
  operations() {
    const cooldownUntil = this.messageCooldownUntil();
    return [...this.pending.values()]
      .sort((left, right) => left.createdAt - right.createdAt)
      .map((pending) => {
        const targetUrl = "conversationUrl" in pending.command
          ? pending.command.conversationUrl
          : pending.command.targetUrl;
        return {
          id: pending.command.id,
          feature: pending.command.feature,
          kind: pending.command.kind,
          label: pending.operation?.label ?? this.defaultOperationLabel(pending.command),
          targetUrl,
          state: pending.claimedBy
            ? "running" as const
            : pending.command.kind === "send_message" && cooldownUntil
              ? "cooldown" as const
              : "queued" as const,
          createdAt: new Date(pending.createdAt).toISOString(),
          ...(pending.operation?.parentThreadId ? { parentThreadId: pending.operation.parentThreadId } : {}),
          ...(pending.operation?.reviewJobId ? { reviewJobId: pending.operation.reviewJobId } : {}),
          ...(pending.command.kind === "send_message" && cooldownUntil ? { retryAt: new Date(cooldownUntil).toISOString() } : {}),
        };
      });
  }

  cancel(commandId: string, reason = "ChatGPT support command cancelled by user.") {
    const pending = this.pending.get(commandId);
    if (!pending) return false;
    this.removePending(commandId);
    pending.reject(new Error(reason));
    this.resetMessagePacingIfIdle();
    return true;
  }

  cancelForReview(reviewJobId: string) {
    return this.cancelMatching((pending) => pending.operation?.reviewJobId === reviewJobId,
      "Reviewer automation cancelled by user.");
  }

  cancelForThread(threadId: string) {
    return this.cancelMatching((pending) => {
      if (pending.operation?.parentThreadId === threadId) return true;
      const targetUrl = "conversationUrl" in pending.command ? pending.command.conversationUrl : pending.command.targetUrl;
      try { return parseConversationUrl(targetUrl).threadId === threadId; }
      catch { return false; }
    }, "Thread automation cancelled because the RALPH thread was marked complete.");
  }
  ...
  private cancelMatching(predicate: (pending: PendingCommand) => boolean, reason: string) {
    let count = 0;
    for (const pending of [...this.pending.values()]) {
  ...
  private defaultOperationLabel(command: SupportCommand) {
    switch (command.kind) {
      case "send_message": return "Send ChatGPT message";
      case "inspect_thread": return "Inspect ChatGPT thread";
      case "prepare_thread": return "Prepare ChatGPT thread";
      case "stop_thread": return "Stop ChatGPT thread";
      case "close_thread": return "Close ChatGPT thread";
    }
  }

  private resetMessagePacingIfIdle() {
    if ([...this.pending.values()].some((entry) => entry.command.kind === "send_message")) return;
  ...
  private browserCanClaim(browserId: string, command: SupportCommand) {
    ...
    if (command.kind === "inspect_thread") {
      ...
      // Prefer the working browser. Its page contains the latest local turn state.
...
export class RalphController {
  ...
  private async check(thread: z.infer<typeof ralphThreadSchema>) {
    try {
      ...
      let inspection = commandResult.result;
      ...
      if (!observedByExecutor) {
        const authoritative = await this.inspectExecutorBeforeDecision(thread);
  ...
  private async inspectExecutorBeforeDecision(thread: z.infer<typeof ralphThreadSchema>) {
    ...
    if (result.kind !== "inspect_thread") throw new Error("RALPH received the wrong authoritative inspection result.");
...
export function ralphThreadsGetHandler(
  registry: RalphRegistry,
  extensionToken: string,
  jobs?: SubagentJobRegistry,
  commands?: SupportCommandBus,
  ...
  return async (req, res) => {
    ...
    res.json({
      threads: threads.map((thread) => ({
        ...thread,
        waitingForReview: thread.state === "active" && reviews.some((job) => job.parentThreadId === thread.threadId &&
...
export function reviewActionHandler(
  ...
  registry: RalphRegistry,
  commands: SupportCommandBus,
  _launchBrowser: () => Promise<void>,
  extensionToken: string,
): RequestHandler {
  return async (req, res) => {
    if (!authenticateSupportExtension(req, res, extensionToken)) return;
    const input = z.object({
      action: z.enum(["cancel", "complete", "retry"]),
    ...
    try {
      ...
      if (input.data.action === "retry") {
        ...
        commands.cancelForReview(job.jobId);
        if (job.childThreadId) await registry.recordComplete(job.childThreadId);
        await jobs.resolve(job.jobId, input.data.action === "cancel" ? "cancelled" : "completed_manually");

        if (input.data.action === "cancel" && job.childConversationUrl && commands.hasBrowser("threadMessaging")) {
          void commands.execute(
            { feature: "threadMessaging", kind: "stop_thread", targetUrl: job.childConversationUrl },
            30_000,
            {
              allowBrowserLaunch: false,
              operation: { label: "Stop cancelled reviewer", reviewJobId: job.jobId, parentThreadId: job.parentThreadId },
            },
          ).then((result) => {
            if (!result.ok) console.warn(`[chatgpt-support] reviewer_stop_failed job=${job.jobId}: ${result.error}`);
          }, (error) => {
            console.warn(`[chatgpt-support] reviewer_stop_failed job=${job.jobId}: ${error instanceof Error ? error.message : String(error)}`);
          });
        }
      }
      res.setHeader("Cache-Control", "no-store");
...
export function ralphThreadCompleteHandler(
  registry: RalphRegistry,
  extensionToken: string,
  commands?: SupportCommandBus,
): RequestHandler {
  return async (req, res) => {
    ...
    commands?.cancelForThread(parsed.data);
    res.setHeader("Cache-Control", "no-store");
...
export function supportCommandCancelHandler(commands: SupportCommandBus, extensionToken: string): RequestHandler {
  return (req, res) => {
    if (!authenticateSupportExtension(req, res, extensionToken)) return;
    const parsed = z.string().uuid().safeParse(req.params.commandId);
    if (!parsed.success) {
      res.status(400).json({ error: "Invalid support command id." });
      return;
    }
    if (!commands.cancel(parsed.data)) {
      res.status(404).json({ error: "Support command is no longer pending." });
      return;
    }
    res.setHeader("Cache-Control", "no-store");
...
function subagentPrompt(message: string, jobId: string, resultPath: string) {
  return [
    ...
    "Call review_done directly with the supplied job ID. No reviewer thread sync or URL lookup is needed; the application already records this reviewer's conversation.",
...
export function registerChatGptAgents(
    ...
    return await replayToolRequest(replayKey, async () => {
      ...
      try {
        ...
        result = await commands.execute({
          ...
          operation: { label: "Start reviewer", reviewJobId: job.jobId, parentThreadId: parent.threadId },
        });
        if (!result.ok) throw new Error(result.error);
  ...
  server.registerTool("review_done", {
    title: "Review done",
    description: "Finish this review using the supplied job ID and complete report. No thread sync or URL lookup is required. The service stores the report locally, waits for the recorded reviewer to become idle, then wakes the implementer. End your turn immediately after success. Submit once. A cancelled review rejects late results.",
    inputSchema: {
      jobId: z.string().uuid(),
    ...
    if (!job?.childThreadId) {
      return {
        isError: true,
        content: [{ type: "text", text: "This review job has no recorded reviewer conversation." }],
      };
    }
    // Reuse an existing binding as a cross-thread guard without requiring one.
    const child = typeof session === "string" && session.length > 0 && session.length <= 2048
      ? await bindings.binding({ ownerId, sessionId: session })
      : undefined;
    if (child && job.childThreadId !== child.threadId) {
      return {
        isError: true,
    ...
    await registry.recordComplete(job.childThreadId);
    ...
    try {
      ...
      return await replayToolRequest(`start_thread:${ownerId}:${String(session)}:${String(extra.requestId)}:${fingerprint}`, async () => {
        ...
        const result = await commands.execute(
          { feature: "threadMessaging", kind: "send_message", targetUrl, message },
          COMMAND_TIMEOUT_MS,
          { operation: { label: "Start ChatGPT thread" } },
        );
        if (!result.ok) throw new Error(result.error);
    ...
    return await replayToolRequest(replayKey, async (): Promise<CallToolResult> => {
      try {
        ...
        const result = await commands.execute({
          ...
          operation: { label: "Send thread message" },
        });
        if (!result.ok) throw new Error(result.error);
...
export class SubagentResultController {
  ...
  private async parentMarkedComplete(parentId: string) {
    if (!this.registry) return false;
    const parent = (await this.registry.threads()).find((thread) => thread.threadId === parentId);
  ...
  private async check(parentId: string) {
    ...
    if (await this.parentMarkedComplete(parentId)) {
    ...
    try {
      ...
      const wake = await this.commands.execute({
        ...
        operation: { label: "Resume parent after review", parentThreadId: parentId },
      });
