...
The separate **Local Codex Support** extension handles thread sync, RALF, and sub-agent messaging without using the general browser-control extension. Thread lookup remains a strict `sync_current_thread` then `get_current_thread_url` sequence.

Delegation now uses first-class tools. `start_subagent` creates the child in the **Sub-agent project** configured on the Local Codex server, or at `https://chatgpt.com/` when no dedicated project is configured. The parent must be synced first; the server reads that parent binding from `openai/session` and automatically adds the parent URL plus `send_thread_message` callback instructions to the child task. `send_thread_message` posts only to an existing conversation. `list_subagents` renders an MCP App with the current parent's children and their status.

Agent-created children are registered for RALF immediately and retain their parent thread ID. The extension captures ChatGPT's readable thread title so the RALF popup can show names instead of UUIDs. Every fresh RALF registration schedules a background tab in the browser that owns RALPH automation. That tab stays open for up to two minutes so the agent can mount Thread Sync and bind itself. A successful sync closes the parked tab early. Duplicate observations and reactivations do not open another initial sync tab.

RALF remains a continuation runtime. Normal threads use the existing worked-duration gate and completion classifier. Each thread also has an explicit **Run continuously** control. Continuous mode skips completion classification and keeps sending a fixed continuation instruction whenever the thread becomes idle. It never turns on automatically. **Stop continuous** returns the thread to normal completion behavior.

The `engineering-loop` skill owns the engineering lifecycle. It routes failures through `diagnosing-bugs`, requires real behavioral verification, delegates substantial changes to an independent `reviewer` sub-agent, fixes and reverifies findings, and uses `git-commit` and `file-pr` when publishing is requested. `autoresearch` remains explicit and can use RALF continuous mode only when the user deliberately starts an indefinite research run.

Run `pnpm support:prepare` to generate `.data/support-extension`. RALF also needs `OPENAI_API_KEY` for normal completion classification. See [support-extension/README.md](support-extension/README.md) and [ADR 0001](docs/adr/0001-explicit-chatgpt-url-binding.md) for the detailed behavior.
