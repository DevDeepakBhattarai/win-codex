...
RALF registration is project-scoped and independent of thread sync. Add one or more ChatGPT project home URLs or project IDs under **RALF projects** in the extension popup. When ChatGPT enters a conversation inside one of those projects, including a thread created by `chatgpt_message`, the extension reports the saved `/g/<project-id>/c/<thread-id>` URL and the server registers it. If an idle project thread changes from a send button to a stop button after a new message, the extension reactivates its existing RALF entry and starts a fresh loop interval. Initial page hydration directly into a running state does not count. Merely reopening a completed thread does not reactivate it. Normal chats and projects not on the list are ignored. Named project home routes such as `/g/g-p-...-deepak/project` are canonicalized to the stable `g-p-...` project ID. **RALF loop interval** controls when the server checks active threads and defaults to 1500 seconds (25 minutes). **RALF minimum worked time** controls the separate `Worked for ...` duration gate and defaults to 1140 seconds (19 minutes). Saving a new loop interval reschedules active threads. When a thread is due, RALF opens it in the enabled browser, waits for the hydrated conversation DOM to become stable, returns immediately if the stop button says the thread is still running, and otherwise reads ChatGPT's duration. Shorter or unknown durations are treated as normal completion.

The **RALF threads** tab has separate Active and Completed views. Use **Mark complete** to stop an active thread, or **Mark active** to reschedule a completed thread for the loop. On a ChatGPT conversation inside a RALF project, **Run RALF now** makes that active thread due immediately. RALF starts its normal inspection and schedules the following check from the result.

Every OpenAI classification call is recorded in `<DATA_DIR>/ralf-openai.log`, which defaults to `.data/ralf-openai.log`, as newline-delimited JSON. RALF requests low reasoning effort and does not set an output-token limit. The start record contains the exact request body, and the result record contains the exact response body, request ID, HTTP status, duration, token usage, and selected action. The API key is never written. Local Codex writes the start record before calling OpenAI, so an unavailable audit log blocks the billable request.
