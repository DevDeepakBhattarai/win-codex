...
The `chatgpt_message` MCP tool uses the same support extension to start a new project thread or send a message to an existing conversation URL. New agent threads must target a ChatGPT `/g/.../project` page; bare `https://chatgpt.com/` new chats are rejected so sub-agents are always spawned under a project. It opens a background ChatGPT tab, waits for the composer and send state to settle, then performs a 30-second bounded send attempt. If the first click is ignored, it retries once only when the unchanged composer proves a duplicate send is safe, captures the resulting saved conversation URL, and closes the automation tab.
