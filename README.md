...
The separate **Local Codex Support** extension handles three ChatGPT-specific features without using the browser-control extension: thread sync, RALPH automation, and agent thread messaging. Thread sync keeps the existing `sync_current_thread` then `get_current_thread_url` flow and can stay enabled in both Chrome and Helium.

The extension popup has independent toggles for **Thread sync**, **RALPH automation**, and **Agent thread messaging**. Enable the two automation features only in the browser you want Local Codex to drive. Commands are also claimed atomically by the server, so two enabled browser instances cannot execute the same command.

RALPH registration is project-scoped and independent of thread sync. Add one or more ChatGPT project home URLs or project IDs under **RALPH projects** in the extension popup. When ChatGPT enters a conversation inside one of those projects, the extension reports the saved `/g/<project-id>/c/<thread-id>` URL and the server registers it. Threads created by AI through `chatgpt_message` are registered for RALPH immediately even when their project is not in this allowlist. If an idle project thread changes from a send button to a stop button after a new message, the extension reactivates its existing RALPH entry and starts a fresh initial delay. Initial page hydration directly into a running state does not count. Merely reopening a completed thread does not reactivate it. Normal chats and projects not on the list are ignored. Named project home routes such as `/g/g-p-...-deepak/project` are canonicalized to the stable `g-p-...` project ID. **RALPH initial check delay** defaults to 1500 seconds (25 minutes). After the first inspection, RALPH checks a still-running thread every 300 seconds (5 minutes). **RALPH minimum worked time** controls the separate `Worked for ...` duration gate and defaults to 1140 seconds (19 minutes). Saving a new initial delay reschedules active threads. When a thread is due, RALPH opens it in the enabled browser, waits for the hydrated conversation DOM to become stable, returns immediately if the stop button says the thread is still running, and otherwise reads ChatGPT's duration. Shorter or unknown durations are treated as normal completion.

The **RALPH threads** tab has separate Active and Completed views. Each active thread has a **Check now** button that makes it due immediately and starts the normal inspection. Use **Mark complete** to stop an active thread, or **Mark active** to reschedule a completed thread for the loop. The following check is scheduled from the inspection result.

Every OpenAI classification call is recorded in `<DATA_DIR>/ralph-openai.log`, which defaults to `.data/ralph-openai.log`, as newline-delimited JSON. RALPH requests low reasoning effort and does not set an output-token limit. The start record contains the exact request body, and the result record contains the exact response body, request ID, HTTP status, duration, token usage, and selected action. The API key is never written. Local Codex writes the start record before calling OpenAI, so an unavailable audit log blocks the billable request.

The `chatgpt_message` MCP tool uses the same support extension to start a new project thread or send a message to an existing conversation URL. Configure the browser's **Sub-agent project** once in the extension popup, then omit `targetUrl` when starting a new sub-agent. Existing conversation URLs and explicit project URLs can still be passed as targets. Bare `https://chatgpt.com/` new chats are rejected so sub-agents are always spawned under a project. It opens a background ChatGPT tab and waits without a local deadline for the composer and send state to settle. If ChatGPT does not acknowledge the first click within 30 seconds, it retries once only when the unchanged composer proves a duplicate send is safe, captures the resulting saved conversation URL, and closes the automation tab.
