...
const CANCEL_AUTOMATION_MESSAGE = "local-codex-support/cancel-automation-v1";
...
async function cancelPageOperations(predicate) {
  const operations = loadedOperations.filter(predicate);
  await Promise.allSettled(operations.map((operation) =>
    extensionApi.runtime.sendMessage({ type: CANCEL_AUTOMATION_MESSAGE, commandId: operation.id })));
}
...
async function setThreadState(thread, button) {
  ...
  try {
    if (nextState === "complete") {
      const normalizedThreadUrl = conversationUrl(thread.conversationUrl);
      await cancelPageOperations((operation) => operation.parentThreadId === thread.threadId ||
        conversationUrl(operation.targetUrl) === normalizedThreadUrl);
    }
...
function renderReview(job) {
  ...
  const label = job.notifiedAt
    ? job.resolution === "completed_manually" ? "Completed manually"
      : job.state === "cancelled" ? "Cancelled" : "Delivered"
    : job.preparationError || job.notificationError ? "Needs attention"
  ...
  const unresolved = job.state === "pending" || (job.state === "complete" && !job.notifiedAt);
  if (unresolved) {
    const actions = Object.assign(document.createElement("div"), { className: "thread-actions" });
    const cancelButton = Object.assign(document.createElement("button"), {
      className: "button button-ghost", type: "button", textContent: "Cancel review",
    });
    cancelButton.addEventListener("click", () => void changeReview(job, "cancel", cancelButton));
    const completeButton = Object.assign(document.createElement("button"), {
      className: "button", type: "button", textContent: "Mark complete",
    });
    completeButton.addEventListener("click", () => void changeReview(job, "complete", completeButton));
    actions.append(cancelButton, completeButton);
    if (job.state === "complete" && job.notificationAbandonedAt) {
      const retryButton = Object.assign(document.createElement("button"), {
        className: "button button-ghost", type: "button", textContent: "Retry parent wake-up",
      });
      retryButton.addEventListener("click", () => void changeReview(job, "retry", retryButton));
...
function renderOperations() {
  const section = element("automationOperationsSection");
  section.hidden = loadedOperations.length === 0;
  element("automationOperationCount").textContent = String(loadedOperations.length);
  const list = element("automationOperationList");
  list.replaceChildren(...loadedOperations.map(renderOperation));
}
...
function renderOperation(operation) {
  ...
  const card = Object.assign(document.createElement("div"), { className: "thread" });
  card.append(
    Object.assign(document.createElement("strong"), { className: "thread-id", textContent: operation.label }),
    Object.assign(document.createElement("p"), { className: "thread-meta", textContent: operation.state === "cooldown" ? "Waiting to retry" : operation.state }),
    Object.assign(document.createElement("p"), { className: "thread-url", textContent: operation.targetUrl }),
  );
  const actions = Object.assign(document.createElement("div"), { className: "thread-actions" });
  const stopButton = Object.assign(document.createElement("button"), {
    className: "button button-ghost", type: "button", textContent: "Stop",
  });
  stopButton.addEventListener("click", () => void stopOperation(operation, stopButton));
...
async function stopOperation(operation, button) {
  ...
  try {
    await cancelPageOperations((candidate) => candidate.id === operation.id);
    const endpoint = new URL(ralphThreadsEndpoint.href);
    endpoint.pathname = `/chatgpt-support/commands/${encodeURIComponent(operation.id)}/cancel`;
    await callServer(endpoint, { method: "PUT" });
    await loadThreads();
  } catch (error) {
    button.disabled = false;
    button.textContent = "Stop";
    setNote(element("threadsStatus"), errorMessage(error, "Could not stop the automation operation."), "error");
  }
}

async function changeReview(job, action, button) {
  if (action === "cancel" && !globalThis.confirm("Cancel this review? Local Codex will stop waiting immediately and will stop the reviewer too when the browser is available.")) return;
  if (action === "complete" && !globalThis.confirm("Mark this review complete? Local Codex will stop waiting for it and suppress any pending parent wake-up.")) return;
  button.disabled = true;
  try {
    await cancelPageOperations((operation) => operation.reviewJobId === job.jobId);
    const endpoint = new URL(ralphThreadsEndpoint.href);
    endpoint.pathname = `/chatgpt-support/reviews/${encodeURIComponent(job.jobId)}`;
    await callServer(endpoint, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) }, 9 * 60_000);
    await loadThreads();
  } catch (error) {
...
async function loadThreads() {
  ...
  button.disabled = true;
  try {
    const { threads, reviews = [], operations = [] } = await callServer(ralphThreadsEndpoint);
